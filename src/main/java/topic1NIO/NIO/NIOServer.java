...
public class NIOServer {
    ...
    private void initServer() throws IOException {
        ...
        InetSocketAddress isa = new InetSocketAddress("127.0.0.1", 10101);
    ...
    private void listen() throws IOException {
        ...
        while (true) {
            ...
            // selctor.selectedKeys集合，获取到 ServerSocketChannel 注册的信息 SelectionKey 后将其从集合中删除，因为 ServerSocketChannel
            // 的 ACCEPT IO操作准备就绪(且对ACCEPT感兴趣) 即 (readyOps() & OP_ACCEPT) != 0，所以 isAcceptable，从而获取 SocketChannel，注册到 selector，
            // 注册时提供 SelectionKey.OP_READ 表示对该ScketChannel 的 RAED IO 操作感兴趣如果，SocketChannel没有发送消息，即IO操作未准备就绪，则selector.select
        ...
        if (socketChannel.read(buffer) > 0) {
            ...
            for (SelectionKey key : selector.keys()) {
                ...
                if (target instanceof SocketChannel) {
                    ...
                    if(targetSocket != socketChannel) targetSocket.write(charset.encode(msg));  // 不向自己发送数据
                }
            }
